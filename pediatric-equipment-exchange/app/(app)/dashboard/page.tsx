...
export default async function Dashboard({ searchParams }: any) {
  ...
  const { user, role, full_name, email} = await getUserAndRole();
 
  if (role === "admin") {
    return <AdminPage user={user} role={role} full_name={full_name} email={email?? "missing"} searchParams={searchParams} />

  } else if (role === "therapist") {
    return <TherapistPage user={user} role={role} full_name={full_name} email={email?? "missing"} />

  } else if (role === "volunteer"){
    return <VolunteerPage user={user} role={role} full_name={full_name} email={email?? "missing"} />
  }
  else {
     return null;
  }
